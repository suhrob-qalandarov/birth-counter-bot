...
public class UpdateBotSessionFunction implements HandlerFunction<BotSession, BotSession> {

    private final BotSessionDatasource botSessionDatasource;

    @Override
    public EDatabaseFunction getMethod() {
        return EDatabaseFunction.BOT_SESSION_UPDATE;
    }

    @Override
    public BotSession execute(BotSession param) {
        return botSessionDatasource.update(param.getId(), param);
    }
}
